...
function calculateBudget() {
    ...
    const savings = {
        ...
        debtPayments: parseFloat(document.getElementById('debt-payments').value) || 0
    };
    ...
    document.getElementById('needs-recommended').textContent = `$${recommendedNeeds.toFixed(2)}`;
    document.getElementById('needs-actual').textContent = `$${needsTotal.toFixed(2)}`;
    document.getElementById('wants-recommended').textContent = `$${recommendedWants.toFixed(2)}`;
    document.getElementById('wants-actual').textContent = `$${wantsTotal.toFixed(2)}`;
    document.getElementById('savings-recommended').textContent = `$${recommendedSavings.toFixed(2)}`;
    ...
    document.getElementById('needs-bar').style.width = `${Math.min(needsPercentage, 100)}%`;
    document.getElementById('wants-bar').style.width = `${Math.min(wantsPercentage, 100)}%`;
    document.getElementById('savings-bar').style.width = `${Math.min(savingsPercentage, 100)}%`;
    
    // Update budget tips
    updateBudgetTips(needsPercentage, wantsPercentage, savingsPercentage, remaining);
...
function updateBudgetTips(needsPercentage, wantsPercentage, savingsPercentage, remaining) {
    const tipsList = document.getElementById('budget-tips-list');
    tipsList.innerHTML = '';
    
    // Add default tips
    const defaultTips = [
        'Start by tracking your expenses for a month to get a clear picture of your spending habits.',
        'Aim to keep your needs at or below 50% of your income.',
        'Try to save at least 20% of your income for future goals and emergencies.'
    ];
    
    // Add specific tips based on budget analysis
    if (needsPercentage > 50) {
        defaultTips.push('Your spending on needs is above the recommended 50%. Consider finding ways to reduce essential expenses, such as getting a roommate or finding cheaper transportation options.');
    }
    
    if (wantsPercentage > 30) {
        defaultTips.push('Your spending on wants is above the recommended 30%. Look for areas where you can cut back on non-essential spending.');
    }
    
    if (savingsPercentage < 20) {
        defaultTips.push('You\'re saving less than the recommended 20%. Try to increase your savings rate to build financial security.');
    }
    
    if (remaining < 0) {
        defaultTips.push('You\'re spending more than you earn. This is unsustainable and will lead to debt. Find ways to increase income or reduce expenses.');
    } else if (remaining > 0.2 * parseFloat(document.getElementById('income').value)) {
        defaultTips.push('You have a significant amount unallocated. Consider putting this extra money toward savings or debt repayment.');
    }
...
function calculateSavings() {
    ...
    if (interestRateDecimal === 0 && contribution === 0 && initial < goal) {
        document.getElementById('time-to-goal').textContent = 'Impossible without contributions or interest';
    ...
    if (initial >= goal) {
        document.getElementById('time-to-goal').textContent = '0 months (Goal already met)';
        document.getElementById('total-contributions').textContent = '$0.00';
        document.getElementById('interest-earned').textContent = '$0.00';
    ...
    while (balance < goal && months < 600) { // Cap at 50 years
        ...
        if (months % 6 === 0 || balance >= goal) {
            const timeLabel = months <= 12 ? `${months} months` : `${Math.floor(months / 12)} years ${months % 12} months`;
    ...
    if (months < 600) {
        if (months <= 12) {
            ...
            timeToGoal = remainingMonths > 0 ? `${years} years, ${remainingMonths} months` : `${years} years`;
        }
    } else {
    ...
    document.getElementById('total-contributions').textContent = `$${(initial + totalContributions).toFixed(2)}`;
    document.getElementById('interest-earned').textContent = `$${totalInterest.toFixed(2)}`;
    
    // Update chart
    updateSavingsChart(chartData.labels, chartData.balances, chartData.contributions, chartData.interest);
...
function calculateSubscriptions() {
    ...
    subscriptionItems.forEach((item, index) => {
        ...
        const cost = parseFloat(document.getElementById(`sub-cost-${id}`).value) || 0;
        const category = document.getElementById(`sub-category-${id}`).value;
        const essential = document.getElementById(`sub-essential-${id}`).value === 'yes';
...
function updateSubscriptionTable(subscriptions) {
    ...
    subscriptions.forEach(sub => {
        ...
        categoryCell.textContent = sub.category.charAt(0).toUpperCase() + sub.category.slice(1);
